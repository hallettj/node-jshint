...
function _lint(file, buffer, results, config, data) {
    var lintdata;
...
module.exports = {
    hint: function (targets, config, reporter, ignore) {
        ...
        var buffersPromise = files.reduce(function(done, file) {
            var bufferDeferred = Q.defer();

            Q.when(done, function() {
                Q.when(_read(file), function(buffer) {
                    bufferDeferred.resolve(buffer);
                });
            }, function(/* error */) {
                bufferDeferred.resolve({ file: file, buffer: '' });
            });

            return Q.join(done, bufferDeferred.promise, function(buffers, buffer) {
                return buffers.concat({ file: file, buffer: buffer });
            });
        }, Q.ref([]));

        var resultsPromise = Q.when(buffersPromise, function(buffers) {
            buffers.forEach(function (buffer) {
                _lint(buffer.file, buffer.buffer, results, config, data);
            });
